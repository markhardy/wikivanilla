...
(function(){
	...
	***************************************************************************/
	...
	function getCreature(search) {
		...
		var url = host + "/npcsearch?search=" + validated;
	...
	function getQuest(search) {
		...
		var url = host + "/questsearch?search=" + validated;
...
*/
	...
	***************************************************************************/
	...
	function displayQuestSearchResults(json_results, requested) {
		var quest_results = document.getElementById("quest_results");
		...
		if (query_results.length == 0) {
			...
			for (var query_result of query_results) {
				var row = document.createElement("tr");
				row.innerHTML = "<td>" + query_result["Title"] + "</td><td>" + query_result["QuestLevel"] + "</td><td>" + query_result["MinLevel"] + "</td>";
